...
public class bookemerald extends AppCompatActivity implements SeatAdapter.OnSeatClickListener {
                        ...
                        if (!snapshot.exists()) {
                            seatRef.setValue(seat)
                                    .addOnSuccessListener(aVoid ->
                                            Toast.makeText(bookemerald.this, "Room initialized", Toast.LENGTH_SHORT).show())
                                    .addOnFailureListener(e ->
                                            Toast.makeText(bookemerald.this, "Failed to initialize room", Toast.LENGTH_SHORT).show());
                        }
                    }

                    @Override
                    public void onCancelled(@NonNull DatabaseError error) {
                        Toast.makeText(bookemerald.this, "Error initializing room", Toast.LENGTH_SHORT).show();
                    }
                });
                ...
                selectedSeatRef.child("count").get().addOnCompleteListener(task->
                        {
                            ...
                            {
                                Toast.makeText(bookemerald.this,"FireBase Error",Toast.LENGTH_SHORT).show();
                            }
                        }
                            ...
                            if(task.isSuccessful())
                            {
                                ...
                                if(Objects.equals(RoomNumber, "null") && count!=0)
                                {
                                    ...
                                    selectedSeatRef.updateChildren(seatFB)
                                            .addOnSuccessListener(aVoid->{
                                                Toast.makeText(bookemerald.this,"Successfully Booked",Toast.LENGTH_SHORT).show();
                                            })
                                            .addOnFailureListener(e->{
                                                Toast.makeText(bookemerald.this,"Try Again",Toast.LENGTH_SHORT).show();
                                            });

                                }
                                else if(count==0)
                                {
                                    Toast.makeText(bookemerald.this,"The Room Is Full",Toast.LENGTH_SHORT).show();
                                }
                                else
                                {
                                    Toast.makeText(bookemerald.this,"You Have Already Booked A Room",Toast.LENGTH_SHORT).show();
                                }
                            }
                            else
                            {
    ...
    private void loadSeatsFromFirebase() {
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(bookemerald.this, "Failed to load seats", Toast.LENGTH_SHORT).show();
            }
        });
    }
