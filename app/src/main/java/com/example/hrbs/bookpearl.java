...
public class bookpearl extends AppCompatActivity implements SeatAdapter.OnSeatClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_bookpearl);
        ImageView close=findViewById(R.id.closeIcon);
        close.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(bookpearl.this, pearl.class);
        ...
        databaseReference = FirebaseDatabase.getInstance().getReference("hostel3").child("pearl");
                        ...
                        if (!snapshot.exists()) {
                            seatRef.setValue(seat)
                                    .addOnSuccessListener(aVoid ->
                                            Toast.makeText(bookpearl.this, "Room initialized Successfully", Toast.LENGTH_SHORT).show())
                                    .addOnFailureListener(e ->
                                            Toast.makeText(bookpearl.this, "Failed to initialize room", Toast.LENGTH_SHORT).show());
                        }
                    }

                    @Override
                    public void onCancelled(@NonNull DatabaseError error) {
                        Toast.makeText(bookpearl.this, "Error initializing room", Toast.LENGTH_SHORT).show();
                    }
                });
        ...
        bookSapphirebtn=findViewById(R.id.bookPearlButton);
                ...
                selectedSeatRef.child("count").get().addOnCompleteListener(task->
                        {
                            ...
                            {
                                Toast.makeText(bookpearl.this,"FireBase Error",Toast.LENGTH_SHORT).show();
                            }
                        }
                            ...
                            if(task.isSuccessful())
                            {
                                ...
                                if(Objects.equals(RoomNumber, "null") && count!=0)
                                {
                                    ...
                                    selectedSeatRef.updateChildren(seatFB)
                                            .addOnSuccessListener(aVoid->{
                                                Toast.makeText(bookpearl.this,"Successfully Booked",Toast.LENGTH_SHORT).show();
                                            })
                                            .addOnFailureListener(e->{
                                                Toast.makeText(bookpearl.this,"Try Again",Toast.LENGTH_SHORT).show();
                                            });

                                }
                                else if(count==0)
                                {
                                    Toast.makeText(bookpearl.this,"The Room Is Full",Toast.LENGTH_SHORT).show();
                                }
                                else
                                {
                                    Toast.makeText(bookpearl.this,"You Have Already Booked A Room",Toast.LENGTH_SHORT).show();
                                }
                            }
                            else
                            {
    ...
    private void loadSeatsFromFirebase() {
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(bookpearl.this, "Failed to load seats", Toast.LENGTH_SHORT).show();
            }
        });
    }
