...
public class bookruby extends AppCompatActivity implements SeatAdapter.OnSeatClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_bookruby);

        ImageView close=findViewById(R.id.closeIcon);
        close.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(bookruby.this, ruby.class);
                        ...
                        if (!snapshot.exists()) {
                            seatRef.setValue(seat)
                                    .addOnSuccessListener(aVoid ->
                                            Toast.makeText(bookruby.this, "Room initialized", Toast.LENGTH_SHORT).show())
                                    .addOnFailureListener(e ->
                                            Toast.makeText(bookruby.this, "Failed to initialize room", Toast.LENGTH_SHORT).show());
                        }
                    }

                    @Override
                    public void onCancelled(@NonNull DatabaseError error) {
                        Toast.makeText(bookruby.this, "Error initializing room", Toast.LENGTH_SHORT).show();
                    }
                });
        ...
        bookSapphirebtn=findViewById(R.id.bookRubyButton);
                ...
                selectedSeatRef.child("count").get().addOnCompleteListener(task->
                        {
                            ...
                            {
                                Toast.makeText(bookruby.this,"FireBase Error",Toast.LENGTH_SHORT).show();
                            }
                        }
                            ...
                            if(task.isSuccessful())
                            {
                                ...
                                if(Objects.equals(RoomNumber, "null") && count!=0)
                                {
                                    ...
                                    selectedSeatRef.updateChildren(seatFB)
                                            .addOnSuccessListener(aVoid->{
                                                Toast.makeText(bookruby.this,"Successfully Booked",Toast.LENGTH_SHORT).show();
                                            })
                                            .addOnFailureListener(e->{
                                                Toast.makeText(bookruby.this,"Try Again",Toast.LENGTH_SHORT).show();
                                            });

                                }
                                else if(count==0)
                                {
                                    Toast.makeText(bookruby.this,"The Room Is Full",Toast.LENGTH_SHORT).show();
                                }
                                else
                                {
                                    Toast.makeText(bookruby.this,"You Have Already Booked A Room",Toast.LENGTH_SHORT).show();
                                }
                            }
                            else
                            {
    ...
    private void loadSeatsFromFirebase() {
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(bookruby.this, "Failed to load seats", Toast.LENGTH_SHORT).show();
            }
        });
    }
